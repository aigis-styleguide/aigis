...
function renderTree(type, tree, depth, root, outputPath) {
  var ul = ejs.compile(
  ...
  var node = ul({
    children: _.map(tree, function(values, name) {
      ...
      if (values.path) {
        relPath = path.join(type, values.path.replace(/\s+/g, '-'), 'index.html');
      ...
      var html = li({
        ...
        children: renderTree(type, values.children, depth + 1, root, outputPath),
